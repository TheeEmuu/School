...
public class Vector
{
    ...
    //Problem #3
    //trimToSize() - this method ensures that the capacit of the Vector is the same as its size
    public void trimToSize() {
    	...
    	//Set the Vector's array to the new (properly sized) array
    	elementData = newElementData;
    }
    
    
    //Problem #10
    //The Object-based remove method - remove the given item (where item is an Object and is returned by the method)
    public Object remove(Object item) {
    	//The removed item (set to null in case item wasn't in the Vector)
    	Object result = null;
    	
    	//The index where we found the element (-1 in case we don't find it)
    	int index = -1;
    	
    	//Loop over the underlying array to look for the item
    	for(int i = 0; i < elementCount; i++) {
    	...
    	//If index >= 0, we found the item!  Remove it using remove method already written
    	if(index >= 0) {
        	result = remove(index);
    	}
