...
{	
    ...
    */
    ...
    /*    public void reverse() {
    	...
    	if(head != null) {
    		//non-empty list
        	int i = count;  //number of items in the list
        	
        	//remove the first item
        	Object val = removeLast();
        	Node n = new Node(val);
        	Node next = null;
        	Node newHead = n;
        	--i;
        	...
        	while(i > 0) {
        		...
        		next = new Node(val);
        		n.setNext(next);  //update the nextElement value
        		n = next;        		
    	...
    	//pre: index < count AND other is nonempty
    	Node n = head;
    	int i = 0;
    	while(i < index) {
    		n = n.next();
    	...
    	Node next = n.next(); //This is where we will pick up from the original list
    	
    	count = count + other.size();
    	int j = other.size()-1;
    	Node otherN = new Node(other.removeFirst());
    	Node otherNext = null;
    	n.setNext(otherN);
    	
    	while(j > 0) {
    		Object val = other.removeFirst();
    		otherNext = new Node(val);
    		otherN.setNext(otherNext);
