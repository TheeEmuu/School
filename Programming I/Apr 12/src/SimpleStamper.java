...
public class SimpleStamper extends JPanel implements MouseListener {
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if(evt.isMetaDown()) {
            Shape newShape = new Shape(x,y,2);
            Shapes.add(newShape);
        }
        else {
            Shape newShape = new Shape(x, y, 1);
            Shapes.add(newShape);
        }
        ...
        for(Shape shape : Shapes){
            switch(shape.type){
                case(1):
                    ...
                    g.fillRect(shape.x, shape.y, 40, 20);
                    break;
                case(2):
                    g.setColor(Color.BLUE);
                    g.fillOval(shape.x, shape.y, 40, 20);
                    break;
            }
        }
    }
...
class Shape{
    public int x,y;
    public int type; //1 for rectangle, 2 for oval

    Shape(int x, int y,int type){
        this.x = x;
        this.y = y;
        this.type = type;
    }
