...
public class RandomTree<E extends Comparable<E>> extends BinaryTree<E> {
	...
	public int compareTo(RandomTree<E> other){
		return this.getRoot().data.compareTo(other.getRoot().data);
	}

	public E max(){
		return recursiveMax(this.getRoot().data, this.getRoot());
	}
	private E recursiveMax(E max, Node<E> cur){
		if(cur.left == null) return max;

		if(cur.data.compareTo(max) > 0) max = cur.data;

		E leftMax = recursiveMax(max, cur.left);
		E rightMax = recursiveMax(max, cur.right);
